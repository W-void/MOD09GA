...
#%%
def detectCloud(b):
    b = cv2.resize(b, None, fx=2, fy=2, interpolation=cv2.INTER_NEAREST)
    flag = (b & 1 == 1) | (b & 2 == 2)
    # # img = np.clip(np.stack((b13, b14, b11), -1)*1e-4, a_min=0, a_max=1).astype(np.float32)
    # # gray = cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)
    # # gray = np.uint8(gray*255)

    # # ret,mask1=cv2.threshold(gray,60,255,cv2.THRESH_BINARY)
    # # ret,mask2=cv2.threshold(gray,20,255,cv2.THRESH_BINARY)
    # # masker = np.zeros_like(mask1, np.int32)
    # # masker += mask1 == 255  # 必是云, 1
    # # masker += (mask2 == 0) * 2  # 必不是云, 2
    # # mark4 = cv2.watershed(np.uint8(img*255) ,masker)    # 返回-1是边界，0是不确定，剩下的就是目标
    # # flag = (b13 < 0) | (mark4 != 2) 
    # flag = (b[4] < -100) | (b[6] > 2000) & (b[2]> 900) & (b[3] > 1000) & (b[0] > 1000)
    return flag


def findBetter(GA, allW):
    ...
    for i in range(2):
        ...
        for i in range(7):
            band = sdFile.select(11+i).get()
            bands.append(band)
            validFlag = validFlag & np.where(band > -101, True, False)

        blueList.append(bands[4])
        state = sdFile.select(1).get()
        flag.append(detectCloud(state) | ~validFlag)
...
#%%
GAPath = 'F:/MOD09GA/'
hdfFileAfterList = [[224, 229], [223, 225], [224, 226], [229, 233]]
hdfFileBeforeList = [[222, 207], [219, 207], [217, 213], [218, 207]]
...
for i in range(4):
    hdfFileBefore = hdfFileBeforeList[i]
    hdfFileAfter = hdfFileAfterList[i]
    region = regionList[i]
    ...
    for i in range(2):
        ...
        for i, word in enumerate(date):
            ...
            b, tmpLandMask = findBetter(zenithList, allW)
            blue[:, :, i] = b
            zz += (tmpLandMask > 0)
        ...
        medianB = blue_sort[yy, xx, np.where(zz//2 > zz-2,  zz//2, zz-2)] # 取中值中的小值
        ...
        writeImage(bandsGA, 'D:\\Data\\MOD09GQ\\output2\\'+date[0].split('.')[1]+'_'+name+'.tiff', \
            geotrans=geotrans, proj=proj)
        writeImage(WI, 'D:\\Data\\MOD09GQ\\output2\\'+date[0].split('.')[1]+'_'+name+'_WI.tiff', \
            geotrans=geotrans, proj=proj)
